...
export default function GestaoEquipe() {
    ...
    async function handleUploadFoto(file: File) {
        ...
        const { error: uploadError } = await supabase.storage
            .from('logos') // Usando o bucket que você já tem
    ...
    async function salvarAtendente() {
        ...
        // Validação de senha APENAS se for um cadastro NOVO
        ...
        try {
            ...
            // 1. Lógica de FOTO (Independente de ser novo ou edição)
            if (foto) {
                urlFinal = await handleUploadFoto(foto)
                // Se já existia uma foto antiga, você pode opcionalmente deletá-la aqui
            }

            // 2. Lógica Híbrida: Atualizar vs. Criar
            if (editando) {
                // MODO EDIÇÃO: Atualiza apenas os dados de perfil
                const { error: dbError } = await supabase.from('atendentes')
                    .update({
                        nome,
                        permissao,
                        ativo,
                        foto_url: urlFinal // Atualiza a URL da foto se ela mudou
                    })
                    .eq('id', editando.id)
                ...
                // MODO NOVO CADASTRO: Cria no Auth e depois no Banco

                // A. Cria no Auth (Exige senha)
                ...
                // B. Cria na tabela pública vinculando o ID
                const { error: dbError } = await supabase.from('atendentes').insert([{
                    auth_user_id: authData.user?.id, // Vínculo essencial para o login
                    nome,
                    email,
                    permissao,
                    ativo: true, // Nasce ativo
                    foto_url: urlFinal // Salva a URL da foto
                }])
            ...
            alert("Erro ao processar a operação: " + (err.message || err.description))
    ...
    async function executarResetSenha(id: string) {
        ...
        // Lógica para update de senha no Auth do Supabase via RPC ou Edge Function
                ...
                <div className="max-w-5xl mx-auto">
                        ...
                        {atendentesFiltrados.map((atendente) => (
                            <div key={atendente.id} className={`bg-card p-5 rounded-2xl border ${atendente.ativo ? 'border-border' : 'border-red-200 opacity-75'} shadow-sm`}>
                                ...
                                <div className="mt-6 flex gap-2">
                                    <button
                                        onClick={() => { setEditando(atendente); setNome(atendente.nome); setEmail(atendente.email); setPermissao(atendente.permissao); setAtivo(atendente.ativo); setShowModal(true); }}
                    ...
                    {showModal && (
                        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                            <div className="bg-card w-full max-w-md rounded-3xl border border-border p-6 md:p-8 shadow-2xl animate-in zoom-in-95 duration-200">
                                ...
                                <div className="flex gap-3 mt-8">
                                    <button onClick={salvarAtendente} disabled={carregando} className="flex-1 bg-primary text-white py-4 rounded-2xl font-black uppercase tracking-widest hover:brightness-110 disabled:opacity-50 transition-all">
                                        {carregando ? 'Salvando...' : editando ? 'Guardar' : 'Criar Atendente'}
                                    </button>
                                    <button onClick={fecharModal} className="px-6 py-4 border border-border rounded-2xl font-black uppercase text-gray-400 hover:bg-muted transition-all text-xs">Sair</button>
                                </div>
                            </div>
                        </div>
                    )}
